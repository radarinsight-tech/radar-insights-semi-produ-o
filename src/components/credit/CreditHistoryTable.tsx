...
import { FileSearch, Download, Trash2, PenLine } from "lucide-react";
...
const statusBadge = (s: string, ajuste: boolean) => {
  if (ajuste) return <Badge className="bg-primary/20 text-primary border border-primary/30 text-xs">Ajuste manual</Badge>;
...
const getTaxaTotal = (entry: CreditHistoryEntry): string => {
  const faixa = normalizeFaixa(entry.decisao_final);
  const taxaMap: Record<string, number> = {
    "ISENTAR": 0, "TAXA_R$100": 100, "TAXA_R$200": 200,
    "TAXA_R$300": 300, "TAXA_R$400": 400, "TAXA_R$1000": 1000,
  };
  const r = entry.resultado as any;
  if (r?.taxa_total !== undefined && r.taxa_total !== null) return formatCurrency(r.taxa_total);
  if (faixa in taxaMap) return formatCurrency(taxaMap[faixa]);
...
const CreditHistoryTable = ({ refreshTrigger }: Props) => {
  ...
  const [filters, setFilters] = useState<CreditFilterValues>({
    usuario: "todos", decisao: "todos", busca: "",
  });
  ...
  const filteredEntries = entries.filter((e) => {
    ...
    if (filters.decisao !== "todos" && normalizeFaixa(e.decisao_final) !== normalizeFaixa(filters.decisao)) return false;
    if (filters.busca) {
  ...
  const handleView = (entry: CreditHistoryEntry) => {
    if (!entry.resultado) { toast.info("Resultado não disponível."); return; }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (error) toast.error("Erro ao excluir consulta.");
      else { toast.success("Consulta excluída."); fetchData(); }
    ...
    <>
          ...
          <Table>
            ...
            <TableBody>
              {loading ? (
                ...
                <TableRow>
                  <TableCell colSpan={9} className="text-center text-muted-foreground py-8">Nenhuma consulta encontrada</TableCell>
                </TableRow>
              ) : (
                filteredEntries.map((e) => (
                  <TableRow key={e.id}>
                    ...
                    <TableCell className="text-sm font-medium">
                      {e.nome || "—"}
                      {e.ajuste_manual && (
                        <Badge variant="outline" className="ml-1.5 text-[10px] border-primary/40 text-primary">Ajustado</Badge>
                      )}
                    </TableCell>
                    <TableCell className="text-sm font-mono">{formatCpfCnpj(e.cpf_cnpj, e.doc_type)}</TableCell>
                    <TableCell className="text-sm">{e.user_name || "—"}</TableCell>
                    <TableCell>{faixaBadge(e.decisao_final)}</TableCell>
                    <TableCell className="text-sm text-muted-foreground max-w-[180px] truncate">{e.regra_aplicada || "—"}</TableCell>
      ...
      {adjustTarget && (
        <CreditAdjustDialog
          ...
          analysisId={adjustTarget.id}
          currentFaixa={adjustTarget.decisao_final}
          clientName={adjustTarget.nome}
